...
[Hadley Wickham's R Style Guide](http://r-pkgs.had.co.nz/style.html) should form the basis for all generated R code, this itself is based on Google's [R Style Guide][1]. Some important points to note are to try to stick to 80 characters per line (exceptions are made for `.Rmd` files or similar) and to use 2 spaces, not tabs, for indentation. With regards to naming conventions, some people prefer `snake_case` where as others prefer `camelCase`; generally speaking either are fine so long as you and any collaborators are consistent with your code.
